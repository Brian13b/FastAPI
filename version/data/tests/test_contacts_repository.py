...
def test_agregar_todosLosCampos_devuelveContacto(db: Session, datosContacto: ContactosRepo):
    payload = Contacto(nombre='nuevo contacto', telefonos='1234567890', fecha_nac=datetime.date(2000, 1, 2), direccion=Direccion(calle='nueva direccion', numero=123))
    data = datosContacto.agregar(db, payload)
    assert busca_contacto_y_compara(db, data, datosContacto)


def test_agregar_DirNone_devuelveContacto(db: Session, datosContacto: ContactosRepo):
    payload = Contacto(nombre='nuevo contacto', telefonos='1234567890', fecha_nac=datetime.date(2000, 1, 2), direccion=None)
    data = datosContacto.agregar(db, payload)
    assert busca_contacto_y_compara(db, data, datosContacto)
...
def test_agregar_locInexistente_errorIntegridad(db: Session, datosContacto: ContactosRepo):
    payload = Contacto(nombre='nuevo contacto', telefonos='1234567890', fecha_nac=datetime.date(2000, 1, 2), direccion=Direccion(calle='nueva direccion', numero=123, localidad_id=99))
